...
namespace NaiveDev.Infrastructure.Jwt
{
    ...
    public class JwtTokenProvider
    {
        ...
        public static async Task<string> BuildTokenAsync(JwtType jwtType, string client, string account, int csrf, DateTime expires)
        {
            return await Task.Run(() =>
            {
                IEnumerable<Claim> claims = [new Claim("type", jwtType.GetDescription()), new Claim("client", client), new Claim("account", account), new Claim("csrf", csrf.ToString())];
        ...
        /// <param name="jwtSecurityToken">如果验证成功，返回解析后的<see cref="JwtSecurityToken"/>对象；否则为null</param>
