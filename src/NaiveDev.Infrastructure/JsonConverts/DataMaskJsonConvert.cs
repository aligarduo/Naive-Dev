...
namespace NaiveDev.Infrastructure.JsonConverts
{
    ...
    /// </remarks>
    public class DataMaskJsonConvert : JsonConverterFactory
    {
        /// <summary>
        /// 确定此转换器是否可以转换指定的类型
        /// </summary>
        ...
        public override bool CanConvert(Type typeToConvert)
        {
            ...
            IEnumerable<MemberInfo> memberInfos = typeToConvert.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
                .Cast<MemberInfo>()
        ...
        public override void Write(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (dataMaskAttribute is not null)
                {
                    ...
                    {
                        maskedValue = CustomRuleMask(dataMaskAttribute.CustomRule, propertyValue?.ToString());
                    }
                    else
                    {
                        ...
                        switch (dataMaskAttribute.MaskMethod)
                        {
                            case MaskMethod.Name:
                                maskedValue = MaskName(propertyValue?.ToString());
                                break;
                            case MaskMethod.Mobile:
                                maskedValue = MaskPhone(propertyValue?.ToString());
                                break;
                            case MaskMethod.Mail:
                                maskedValue = MaskMail(propertyValue?.ToString());
                                break;
                            default:
                                break;
                        }
